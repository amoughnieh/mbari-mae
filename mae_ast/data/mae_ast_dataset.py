...
class MAE_AST_Dataset(FairseqDataset):
    ...
    def postprocess_spec(self, wav, cur_sample_rate):
        ...
        feat = self.mel_spectrogram_transform(wav)

        feat = torchaudio.functional.amplitude_to_DB(
            feat,
            multiplier=10.0,
            amin=1e-10,
            db_multiplier=0,
        ...
        if self.deltas:
            deltas = torchaudio.functional.compute_deltas(feat.transpose(0, 1))
            ddeltas = torchaudio.functional.compute_deltas(deltas)
            # Transpose deltas back to (time, features) before concatenating
            feat = torch.cat([feat, deltas.transpose(0, 1), ddeltas.transpose(0, 1)], dim=1)
